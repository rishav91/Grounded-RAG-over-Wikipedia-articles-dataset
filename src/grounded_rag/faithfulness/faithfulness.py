...
cited chunk, gating the abstain decision (FR6, FR7; ADR-006).
...
def judge_faithfulness(
    ...
    if not citations or not answer:
        return FaithfulnessResult(passed=False, confidence=0.0, reasoning="No citations to verify.", claim_checks=[])
    ...
    passed = judgment.passed and judgment.confidence >= FAITHFULNESS_CONFIDENCE_THRESHOLD
    claim_checks = [
        ClaimCheck(claim=c.claim, chunk_id=c.chunk_id, supported=c.supported) for c in judgment.claim_checks
    ]
    return FaithfulnessResult(
        passed=passed, confidence=judgment.confidence, reasoning=judgment.reasoning, claim_checks=claim_checks
    )
